...
function TestDefaultCommand(testCase)
  ...
  [~, ~, actualCommand] = Multiply(pathToWorkspace, configCell{:});
  ...
  expectedCommand = sprintf('fslmaths %s -mul 2 %s', inputfile, outputfile);
...
function TestSpecifyingAllOptions(testCase)
  ...
  inputfile = fullfile(pathToWorkspace, inputVolume);
  outputfile = fullfile(pathToWorkspace, outputVolume);
  expectedCommand = sprintf('fslmaths %s -mul 3 %s', inputfile, outputfile);
