...
function TestDefaultCommand(testCase)
  ...
  [~, ~, actualCommand] = ApplyWarp(pathToWorkspace, configCell{:});
  ...
  expectedCommand = ...
   sprintf('applywarp --in=%s --ref=%s --out=%s --interp=trilinear', inputfile, referencefile, outputfile);
...
function TestSpecifyingWarpOption(testCase)
  ...
  % this is the expected command with warp option
  inputfile = fullfile(pathToWorkspace, inputVolume);
  warpfile = fullfile(pathToWorkspace, warpVolume);
  referencefile = fullfile(pathToWorkspace, referenceVolume);
  outputfile = fullfile(pathToWorkspace, outputVolume);
  expectedCommand = ...
   sprintf('applywarp --in=%s --ref=%s --out=%s --warp=%s --interp=spline -v', ...
   inputfile, referencefile, outputfile, warpfile);
