...
function TestDefaultCommand(testCase)
  ...
  expectedCommand = ...
   sprintf('invwarp --warp=%s --ref=%s --out=%s --abs --jmin=0.01000 --jmax=100.00000', warpfield, referencefile, outputfile);
...
function TestSpecifyingAllOptions(testCase)
  ...
  referencefile = fullfile(pathToWorkspace, referenceVolume);
  expectedCommand = ...
   sprintf('invwarp --warp=%s --ref=%s --out=%s --rel --jmin=0.05000 --jmax=110.00000 --debug -v', warpfield, referencefile, outputfile);
