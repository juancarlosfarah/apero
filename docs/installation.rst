...
Inside MATLAB, you can then add the ``/src/matlab`` folder and subfolders to
your MATLAB path.

External Dependencies
=====================

*Apéro* is currently written using MATLAB R2021a (or above) and requires external neuroimaging
tools to run. What these tools are depends on the type of operations that are executed.

By default, we include the following MATLAB toolboxes within the `/src/matlab/external` directory,
which can be added to the MATLAB path alongside *Apéro*.

- FreeSurfer_ (v7)
- FsFast (included in FreeSurfer_)
